...
def register():
    if request.method == "POST" :
        ...
        # after a successful registration, redirect it to the profile page
...
def login():
    if request.method == "POST" :
        ...
        if existing_user:
            ...
            if check_password_hash(
                existing_user["password"], request.form.get("password")):
                    # set cookie session for the user
                    session["user"] = request.form.get("username").lower()
                    flash("Welcome, {}".format(
                        request.form.get("username")))
                    # after a successful login, redirect it to the profile page
        ...
        # if the username doesn't exist in db
        else:
            flash("Incorrect Username and/or Password")
            return redirect(url_for("login"))
...
@app.route("/delete_task/<task_id>") # we only use GET method, which is the default method if we don't specify it
